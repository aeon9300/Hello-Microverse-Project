...
const { getCategoryName } = utils_1.i18n;
...
class CategoryResult {
    constructor(name, url, isScanner, language) {
        ...
        this.passed = [];
        this.name = name;
        this.localizedName = getCategoryName(name.toLowerCase(), language);
        this.hintsCount = 0;
    ...
    addProblem(problem) {
        ...
        if (!hint) {
            const status = problem.severity === hint_1.Severity.error ? 'error' : 'warning';
            hint = new HintResult(hintId, status, this.url, this.isScanner);
            this.hints.push(hint);
        }
        if (problem.severity === hint_1.Severity.error || problem.severity === hint_1.Severity.warning) {
            this.hintsCount++;
        }
