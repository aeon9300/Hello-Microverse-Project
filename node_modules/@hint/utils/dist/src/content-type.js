...
exports.getFileExtension = fs_1.fileExtension;
const normalize_string_1 = require("./misc/normalize-string");
...
const determineCharset = (originalCharset, mediaType) => {
    ...
    let determinedCharset = typeInfo && normalize_string_1.normalizeString(typeInfo.charset);
...
const determineMediaTypeForScript = (element) => {
    const typeAttribute = normalize_string_1.normalizeString(element.getAttribute('type') || '');
...
const determineMediaTypeBasedOnElement = (element) => {
    const nodeName = element && normalize_string_1.normalizeString(element.nodeName);
    if (element && nodeName) {
        if (nodeName === 'script') {
...
const determineMediaTypeBasedOnFileExtension = (resource) => {
    const fileExtension = fs_1.fileExtension(resource);
    if (!fileExtension) {
        return null;
    }
...
const determineMediaTypeBasedOnFileName = (resource, rawContent) => {
    const fileName = fs_1.fileName(resource);
    if (!fileName) {
        return null;
    }
...
const parseContentTypeHeader = (headers) => {
    const contentTypeHeaderValue = normalize_string_1.normalizeString(headers ? headers['content-type'] : null);
