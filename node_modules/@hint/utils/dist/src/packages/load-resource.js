...
const resource_error_status_1 = require("../types/resource-error-status");
...
exports.tryToLoadFrom = (resourcePath) => {
    let builder = null;
    try {
        const resource = fs_1.loadJSONFile(resourcePath);
        return resource;
    }
    catch (e) {
    ...
    try {
        ...
        if (e.code === 'MODULE_NOT_FOUND') {
            ...
            throw new resource_error_1.ResourceError(errorMessage, resource_error_status_1.ResourceErrorStatus.DependencyError);
        }
        throw new resource_error_1.ResourceError(e, resource_error_status_1.ResourceErrorStatus.Unknown);
    }
    return builder;
};
...
const generateConfigPathsToResources = (configurations, name, type) => {
    return configurations.reduce((total, configuration) => {
        const basePackagePaths = is_full_package_name_1.isFullPackageName(configuration, resource_type_1.ResourceType.configuration) ?
            [''] :
...
exports.loadResource = (name, type, configurations = [], verifyVersion = false) => {
    ...
    const currentProcessDir = fs_1.cwd();
    let sources;
    if (isSource) {
        ...
        if (res && !isSource) {
            ...
            if (source === currentProcessDir) {
                try {
                    ...
                    if (!misc_1.normalizeIncludes(packageConfig.name, packageName)) {
                        return false;
                    }
                }
                catch (e) {
    ...
    if (!isValid) {
        throw new resource_error_1.ResourceError(`Resource ${name} isn't compatible with current hint version`, resource_error_status_1.ResourceErrorStatus.NotCompatible);
    }
    if (!resource) {
        debug(`Resource ${name} not found`);
        throw new resource_error_1.ResourceError(`Resource ${name} not found`, resource_error_status_1.ResourceErrorStatus.NotFound);
    }
