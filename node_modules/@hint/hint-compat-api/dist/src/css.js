...
const validateDeclValue = (node, context) => {
    const unsupported = utils_compat_data_1.getUnsupportedDetails({ property: node.prop, value: node.value }, context.browsers);
    if (unsupported) {
...
const reportUnsupported = (reportsMap, context) => {
    for (const reports of reportsMap.values()) {
        ...
        const unprefixedReports = reports.filter(({ node }) => {
            switch (node.type) {
                case 'atrule':
                    return !postcss_1.vendor.prefix(node.name);
                case 'decl':
                    return !postcss_1.vendor.prefix(node.prop) && !postcss_1.vendor.prefix(node.value);
                default:
                    return false;
            }
        });
...
const walk = (ast, context) => {
    ...
    for (const node of ast.nodes) {
        ...
        switch (node.type) {
            case 'atrule':
                key = `@${postcss_1.vendor.unprefixed(node.name)} ${node.params}`;
                report = validateAtRule(node, context);
                break;
            case 'comment':
                break;
            case 'decl':
                key = `${postcss_1.vendor.unprefixed(node.prop)}`;
        ...
        context.on('parse::end::css', ({ ast, element, resource }) => {
            ...
            const report = ({ feature, formatFeature, isValue, node, unsupported }) => {
                const message = [
                    ...
                    ...alternatives_1.formatAlternatives(context.language, unsupported, formatFeature)
                ].join(' ');
                const codeSnippet = report_1.getCSSCodeSnippet(node);
                const location = report_1.getCSSLocationFromNode(node, { isValue });
