...
const { traverse } = utils_1.dom;
const { getContentTypeData, getType, isTextMediaType } = utils_1.contentType;
const { cwd, isFile, readFileAsync } = utils_1.fs;
const { asPathString, getAsUri } = utils_1.network;
...
class LocalConnector {
    ...
    async getGitIgnore() {
        try {
            ...
            utils_1.logger.error(i18n_import_1.getMessage('errorReading', this.engine.language));
            return [];
        }
    }
    async notify() {
        ...
        utils_1.logger.log(i18n_import_1.getMessage('watchingForChanges', this.engine.language));
    }
    watch(targetString) {
        return new Promise(async (resolve, reject) => {
            ...
            const onAdd = async (filePath) => {
                ...
                utils_1.logger.log(i18n_import_1.getMessage('fileAdded', this.engine.language, file));
            ...
            const onChange = async (filePath) => {
                ...
                utils_1.logger.log(i18n_import_1.getMessage('fileChanged', this.engine.language, file));
                if (fileUrl) {
            ...
            const onUnlink = async (filePath) => {
                ...
                utils_1.logger.log(i18n_import_1.getMessage('fileDeleted', this.engine.language, file));
                await this.notify();
            };
            const onReady = async () => {
                await this.notify();
            ...
            const onError = (err) => {
                utils_1.logger.error(i18n_import_1.getMessage('error', this.engine.language), err);
                reject(err);
            };
            const onClose = () => {
                if (this.watcher) {
    ...
    async fetchContent(target, headers, options) {
        const uri = getAsUri(target);
        const filePath = uri ? asPathString(uri) : '';
