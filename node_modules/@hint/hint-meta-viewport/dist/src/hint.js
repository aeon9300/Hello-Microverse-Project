    ...
    constructor(context) {
        const getViewportMetaElements = (elements) => {
            return elements.filter((element) => {
                return (element.getAttribute('name') !== null && normalize_string_1.normalizeString(element.getAttribute('name')) === 'viewport');
            });
        };
        ...
        const checkContentValue = (contentValue, resource, viewportMetaElement) => {
            ...
            for (const key of Object.keys(content.invalidValues)) {
                const message = i18n_import_1.getMessage('metaElementInvalidValues', context.language, [content.invalidValues[key].toString(), key]);
                context.report(resource, message, { element: viewportMetaElement });
            }
        ...
        const validate = ({ resource }) => {
            ...
            if ((bodyMetaElements.length > 0) && bodyMetaElements[0].isSame(viewportMetaElement)) {
                context.report(resource, i18n_import_1.getMessage('metaElementInBody', context.language), { element: viewportMetaElement });
            }
            const contentValue = normalize_string_1.normalizeString(viewportMetaElement.getAttribute('content'));
