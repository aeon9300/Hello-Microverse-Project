...
const { isFile, loadJSFile, loadJSONFile } = utils_1.fs;
...
class Configuration {
    ...
    static toAbsolutePaths(config, configRoot) {
        return utils_1.toAbsolutePaths(config, configRoot);
    }
    ...
    static validateConnectorConfig(config) {
        ...
        const Connector = resourceLoader.loadResource(connectorId, enums_1.ResourceType.connector);
        debug(`Connector schema:`);
        debug(Connector.schema);
        debug(`User configuration:`);
        debug(config.connector.options);
        return schema_validator_1.validate(Connector.schema, config.connector.options).valid;
    }
