...
__export(require("@hint/utils/dist/src/packages/load-resource"));
const { readFile } = utils_1.fs;
const { findNodeModulesRoot, findPackageRoot, hasMultipleResources, isFullPackageName, loadResource, requirePackage } = utils_1.packages;
const debug = utils_1.debug(__filename);
const HINT_ROOT = findPackageRoot();
const NODE_MODULES_ROOT = (() => {
    const root = findNodeModulesRoot();
    return root;
})();
...
exports.loadHint = (hintId, configurations) => {
    return loadResource(hintId, resource_type_1.ResourceType.hint, configurations);
};
exports.loadConfiguration = (configurationId, configurations) => {
    return loadResource(configurationId, resource_type_1.ResourceType.configuration, configurations);
};
exports.loadResources = (config) => {
    let connector = null;
    const connectorName = config.connector && config.connector.name || '';
    try {
        connector = loadResource(connectorName, resource_type_1.ResourceType.connector, config.extends, true);
    }
    catch (e) {
        debug(e);
